...
- **Fix-wave escalation:** the Redis `.on("error", ...)` listeners alone did not resolve the intermittent `npm run test` flake — 5 consecutive runs still failed once. The actual mechanism was a rejected in-flight command promise (`Connection is closed.`), not an unhandled `error` event, surfaced by `createApp()`'s duplicated adapter connection (`subClient`) never being explicitly quit in three server test files that call `createApp()` per test (`roleSelectEvents.test.ts`, `roomEvents.test.ts`, `index.test.ts`). The error listeners were kept regardless (correct on their own merits, and relevant to the Upstash-connection-limit risk the review raised for production), and `await app.subClient.quit()` was added to those three files' `afterEach`, matching the teardown `socketAdapter.test.ts` already did correctly. Confirmed resolved: 20 consecutive `vitest run` plus the required 5 consecutive `npm run test -w server`, all clean.
